...
import configparser
from utils.loss import cls_loss, box_constraint_loss, \
    ranking_loss
from utils.visualize_graph import *
from utils.metric import TreeMetric
from utils.utils import checkpoint, sample_triples, sample_n_nodes, conditional_prob, \
    save_state, rearrange, collate, obtain_ranks, rescore_by_chatgpt, partition_array, rescore_by_att
...
parser.add_argument("--load_dataset_pt", type=str, default='wn_small.pt',
                    choices=['wn_food.pt', 'wn_verb.pt', 'mag_cs.pt',
...
def model_sum(model):
    ...
    rich.print("Number of model params: {}".format(num_params))
...
def train(model, dataset, optimizer, scheduler, device, args):
    ...
    best = 0
    not_improve = 0
    total_iters, cum_tot_loss, cum_i_loss, cum_b_loss, cum_r_loss = 0, 0, 0, 0, 0
    ...
    for e in range(args.epoch):
        ...
        # if not_improve == 30:
        #     rich.print("Early stop.")
        #     break

        for i, batch in enumerate(track(dataloader)):
            optimizer.zero_grad()

            queries, p_datas, c_datas, labels, sims, reaches, i_idx, rank_sims = batch
            queries = queries.to(device)
            labels = labels.to(device)
            sims = sims.view(-1, 2).to(device)
            i_idx = i_idx.to(device)
            reaches = reaches.view(-1, 4).to(device)

            boxes, scores = model(queries, p_datas, c_datas, i_idx)
            loss_i = cls_loss(scores, labels)
            loss_r = ranking_loss(scores, labels, rank_sims) if args.loss_r > 0 else 0
            loss_box = box_constraint_loss(boxes.view(boxes.shape[0] * boxes.shape[1], 3, -1), sims,
                                           reaches) if args.loss_b > 0 else 0
            ...
            cum_b_loss += loss_box.item() if hasattr(loss_box, 'item') else 0
            cum_r_loss += loss_r.item() if hasattr(loss_r, 'item') else 0

            total_iters += 1
            if total_iters % 10 == 0:
                rich.print("Epoch {},{}. loss:{}, i_loss:{}, b_loss:{}, r_loss:{}".format(e, total_iters,
                                                                                          cum_tot_loss / 10,
                                                                                          cum_i_loss / 10,
                                                                                          cum_b_loss / 10,
                                                                                          cum_r_loss / 10))
        ...
        res = __test(dataset, model, i_metric, a_metric, 'eval', device,
                     sep_count=args.sep_count) if args.load_dataset_pt != 'conceptnet.pt' else test_conceptnet(dataset,
                                                                                                               model,
                                                                                                               i_metric,
                                                                                                               'eval',
                                                                                                               device)
        scheduler.step(res['mrr'])
        rich.print('Current lr:', optimizer.state_dict()['param_groups'][0]['lr'])
...
def __test(dataset, model, i_metric, a_metric, mode, device, sep_count=False):
    ...
    for b in track(dataset.train, description='Generating seed embeds'):
        ...
        emb = model.forward_graph(p_datas[0].to(device),
                                  torch.tensor([0]).long().to(device)) if model.graph_embed else p_datas[0].x[
    ...
    for i, n in enumerate(track(dataset, description='{}'.format(mode))):
        # dataset.shuffle()
        if dataset.d_name == 'bamboo' and mode == 'eval' and i > 300:
            break
        ...
        # scores = rescore_by_att(scores, att_idx, ins_idx, edge_t)

        scores, labels = rearrange(scores, dataset.edges, labels)
        rank = obtain_ranks(scores, labels)[0]

        if sep_count:
            # separate attachment and insertion
            # all the labels are attachment ones
